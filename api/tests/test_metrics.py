def test_ingest_metric(client, create_device):
...
def test_ingest_metric_unknown_device(client):
    response = client.post("/metrics", json={"device_id": 999, "status": "up"})
    assert response.status_code == 404


def test_ingest_invalid_status(client, create_device):
    device = create_device()
    response = client.post("/metrics", json={"device_id": device["id"], "status": "broken"})
    assert response.status_code == 422


def test_ingest_oversized_raw_output_rejected(client, create_device):
...
def test_get_device_metrics_newest_first(client, create_device):
    device = create_device()
    client.post("/metrics", json={"device_id": device["id"], "status": "up", "cpu_percent": 10})
    client.post("/metrics", json={"device_id": device["id"], "status": "up", "cpu_percent": 20})
...
def test_get_latest_metrics_one_row_per_device(client, create_device):
    ...
    for device in (dev_a, dev_b):
        ...
        client.post("/metrics", json={"device_id": device["id"], "status": "down"})
