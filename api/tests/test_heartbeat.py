...
def test_heartbeat_requires_key(client):
    assert client.post("/v1/poller/heartbeat", json=HEARTBEAT).status_code == 401
...
def test_heartbeat_requires_metrics_write_scope(client, make_api_key):
    ...
    resp = client.post("/v1/poller/heartbeat", json=HEARTBEAT, headers={"X-API-Key": raw_key})
    assert resp.status_code == 403


def test_heartbeat_creates_status_row(client, auth_headers):
    resp = client.post("/v1/poller/heartbeat", json=HEARTBEAT, headers=auth_headers)
    ...
    status = client.get("/v1/poller/status")
...
def test_heartbeat_upserts_single_row(client, auth_headers):
    ...
    first = client.get("/v1/poller/status").json()[0]

    updated = {**HEARTBEAT, "devices_polled": 9, "failures": 0}
    client.post("/v1/poller/heartbeat", json=updated, headers=auth_headers)

    rows = client.get("/v1/poller/status").json()
    assert len(rows) == 1
...
def test_heartbeat_rejects_negative_counts(client, auth_headers):
    ...
    resp = client.post("/v1/poller/heartbeat", json=bad, headers=auth_headers)
