...
def test_write_without_key_returns_401(client):
    assert client.post("/v1/devices", json=DEVICE).status_code == 401


def test_write_with_bogus_key_returns_401(client):
    resp = client.post("/v1/devices", json=DEVICE, headers={"X-API-Key": "nwk_not-a-real-key"})
    assert resp.status_code == 401


def test_revoked_key_returns_401(client, make_api_key):
    raw_key = make_api_key(name="revoked-service", is_active=False)
    resp = client.post("/v1/devices", json=DEVICE, headers={"X-API-Key": raw_key})
    assert resp.status_code == 401
...
def test_scoped_key_can_do_its_job(client, make_api_key, create_device):
    ...
    ingest = client.post(
        "/v1/metrics", json={"device_id": device["id"], "status": "up"}, headers=headers
    )
    assert ingest.status_code == 201

    assert client.delete(f"/v1/devices/{device['id']}", headers=headers).status_code == 403


def test_patch_and_delete_require_key(client, create_device):
    device = create_device()
    assert client.patch(f"/v1/devices/{device['id']}", json={"site": "B"}).status_code == 401
    assert client.delete(f"/v1/devices/{device['id']}").status_code == 401


def test_reads_stay_open(client, create_device):
    """The dashboard has no identity until Phase 6 â€” GETs must work keyless."""
    device = create_device()
    assert client.get("/v1/devices").status_code == 200
    assert client.get(f"/v1/devices/{device['id']}").status_code == 200
    assert client.get("/v1/metrics/latest").status_code == 200
    assert client.get(f"/v1/devices/{device['id']}/metrics").status_code == 200
